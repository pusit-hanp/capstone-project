...
const ImageUpload = () => {
  ...
  const imageUpload = async (values) => {
    ...
    try {
      ...
      formData.append('product_image', values.imageFile);

      await axios.post('/test_upload', formData, {
        headers: {
          'Content-Type': 'multipart/form-data; boundary=----WebKitFormBoundaryUDMrAP3w6RzLvxmu',
        },
      });

      // Successful upload new image, you can navigate to another route if needed
